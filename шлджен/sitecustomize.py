...
def _patched_compile(source,filename,mode,*args,**kwargs):
    if not isinstance(source,str): return _original_compile(source,filename,mode,*args,**kwargs)
...
'''
        ...
        old="""    def blackjack_start(self,uid,bet):\n        if uid in self.blackjack or not self.cost(uid,bet,'21_bet'):return False\n"""
        new="""    def blackjack_start(self,uid,bet):\n        # Re-open the player's unfinished 21 game instead of charging twice.\n        if uid in self.blackjack:return self.blackjack[uid]\n        if not self.cost(uid,bet,'21_bet'):return False\n"""
