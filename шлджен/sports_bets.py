...
def bet_k(uid,game):
    ...
    b.button(text='✍️ Своя ставка',callback_data=f'sportcustom:{uid}:{game}'); b.button(text='◀️ Назад',callback_data=f'sportback:{uid}:{game}'); b.adjust(2,2,1,1); return b.as_markup()
...
async def roll(c,uid,game,choice,amount):
    ...
    mapping=DARTS if game=='darts' else BOWLING; win=value in mapping[choice]; payout=amount*Decimal(db.setting('sport_multiplier') or '2') if win else Decimal(0); games.finish(uid,game,amount,f'{choice};roll:{value}',payout,win)
...
def parse_legacy(d):
    p=d.split(':')
    if d.startswith('gx:') and len(p)>=4:
        try:uid=int(p[1])
        except:return None
        act=p[2]
        if act=='game' and len(p)==4 and p[3] in ('darts','bowling'):return uid,'game',p[3],None
        if act=='bet' and len(p)==5 and p[3] in ('darts','bowling'):
            try:return uid,'bet',p[3],Decimal(p[4])
            except:return None
        if act=='custom' and len(p)==4 and p[3] in ('darts','bowling'):return uid,'custom',p[3],None
    if d.startswith('game:') and len(p)==2 and p[1] in ('darts','bowling'):return None,'game',p[1],None
    if d.startswith('bet:') and len(p)==3 and p[1] in ('darts','bowling'):
        try:return None,'bet',p[1],Decimal(p[2])
        except:return None
    return None

def sport_callback_filter(c):
    d=c.data or ''
    return d.startswith(('sportstart:','sportcustom:','sportbet:','sportback:')) or parse_legacy(d) is not None
...
async def cb(c):
    ...
    legacy=parse_legacy(d)
    if legacy:
        uid,act,game,amount=legacy
        if uid is not None and c.from_user.id!=uid:return await c.answer()
        await c.answer()
        if act=='game':return await show_start(c.message,game)
        if act=='bet':return await show_choice(c.message,game,amount)
        if act=='custom':
            _PENDING[c.from_user.id]=game
            return await c.message.answer(f'✍️ Введи свою ставку для {"дартса" if game=="darts" else "боулинга"} одним сообщением. Например: <code>30000</code>',parse_mode='HTML')
...
async def msg_handler(message):
    ...
    if uid in _PENDING and text and not text.startswith('/'):
        ...
        except:return await message.answer('❌ Ставка должна быть положительным целым числом.')
    ...
    if len(parts)==2:
        ...
        except:return await message.answer('❌ Укажи ставку числом, например: <code>дартс 100</code>',parse_mode='HTML')
        if amount<=0 or amount!=amount.to_integral_value():return await message.answer('❌ Ставка должна быть положительным целым числом.')
        return await show_choice(message,game,amount)
